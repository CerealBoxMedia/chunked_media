...
from knowledge_hub.chunked_media import admin_urls
from knowledge_hub.chunked_media.forms import GroupMediaPermissionFormSet
from knowledge_hub.chunked_media.models import get_media_model
from knowledge_hub.chunked_media.permissions import permission_policy


@hooks.register('register_admin_urls')
def register_admin_urls():
    return [
        url(r'^media/', include((admin_urls, 'chunked_media'), namespace='chunked_media')),
    ]
...
def editor_js():
    return format_html(
        ...
        """,
        reverse('chunked_media:chooser')
    )


class MediaSummaryItem(SummaryItem):
    order = 300
    template = 'chunked_media/homepage/site_summary_media.html'
...
def describe_collection_media(collection):
    ...
    if media_count:
        url = reverse('chunked_media:index') + ('?collection_id=%d' % collection.id)
        return {
            'count': media_count,
